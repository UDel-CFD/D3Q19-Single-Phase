...
!===========================================================================
      ...
      integer ip, ilen, indy9, indz9, i
      integer status(MPI_STATUS_SIZE)
      integer istp1, istp2, istp3, istp4, istp5, istp6, istp7
      real xx0,zz0,bin_space,rr0,vpipe,lam,time
      integer j,k,ks,ibin
      real,allocatable,dimension(:,:,:):: uy9
      real,allocatable,dimension(:,:,:):: uy0
      real,allocatable,dimension(:):: uprofile
      integer,allocatable,dimension(:):: nbin
      integer npoints
      character (len = 120):: fnm,fnm1
      real,dimension(10)::roots
!      real Besj0,Besj1
      ilen = lx*ly*lz
      roots = (/2.40482555769577, 5.52007811028631,    &
                8.65372791291101, 11.7915344390143,    &
                14.9309177084878,18.0710639679109,     &
                21.2116366298793,24.3524715307493,     &
                27.4934791320403,30.6346064684320/)
...
!        where(ibnodes9 > 0) uy9 = 0.0
        ...
        fnm = trim(dirflowout)//'uy_contour'                                   &
            //char(istp1 + 48)//char(istp2 + 48)//char(istp3 + 48)     &
            //char(istp4 + 48)//char(istp5 + 48)//char(istp6 + 48)     &
            //char(istp7 + 48)//'.dat' 
        fnm1 = trim(dirflowout)//'uy_profile'       &
            //char(istp1 + 48)//char(istp2 + 48)//char(istp3 + 48)     &
            //char(istp4 + 48)//char(istp5 + 48)//char(istp6 + 48)     &
            //char(istp7 + 48)//'.dat'
        ...
        open(17, file = trim(fnm), status = 'unknown',                 &
                 form = 'formatted')
        open(18, file = trim(fnm1), status = 'unknown',&
                 form = 'formatted')

        npoints = 50 ! Use 100 points in the profile
          allocate(uprofile(1:npoints))
          allocate(nbin(1:npoints))

          uprofile = 0.d0
          nbin = 0

          bin_space = 1.d0/npoints

        j = ny/2
        do i = 1, nx
          xx0 = dfloat(i)-0.5d0-pxcenter
          do k = 1,nz
          zz0 = dfloat(k)-0.5d0-pzcenter
          rr0 = dsqrt(xx0*xx0+zz0*zz0)/prad
          vpipe = 1.d0 - rr0*rr0

          if (rr0.le.1.d0) then

          ibin = floor(rr0/bin_space) + 1
          if (ibin.lt.1) ibin = 1
          if (ibin.gt.npoints) ibin = npoints

          nbin(ibin) = nbin(ibin) + 1
          uprofile(ibin) = uprofile(ibin) + uy9(i,j,k)/u0


          write(17,170) xx0,zz0,uy9(i,j,k)/u0,vpipe
          else
          write(17,170) xx0,zz0,0.d0,0.d0
          end if
         end do
         end do
         close(17)

!        time = visc*dfloat(istep)/prad/prad

         do i = 1,npoints
           if(nbin(i).gt.0) uprofile(i) = uprofile(i)/dfloat(nbin(i))
           rr0 = (dfloat(i)-0.5d0)*bin_space
! Calculate the theoretical solution for the unsteady pipe flow
!           vpipe = 0.d0
!           do j = 1,10
!           lam = roots(j)
!           vpipe = vpipe + BesJ0(lam*rr0)*    &
!            exp(-lam*lam*time)/lam/lam/lam/BesJ1(lam)
!           end do
!           vpipe = -8.d0*vpipe + (1.d0-rr0*rr0)
           write(18,171) rr0,uprofile(i)
         end do
...
170   format(2x,4(1pe16.6))
171   format(2x,2(1pe16.6))
